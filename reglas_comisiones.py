...
def estandarizar_equipo(valor):
    ...
    if not texto:
        return ""
    if "PINPAGO" in texto or "PIN PAGO" in texto or "EQUIPO K" in texto:
        return "Pinpagos"
    if "ZAPPY" in texto or "SAPPY" in texto or "S1MINI2" in texto or "S1 MINI 2" in texto:
        return "Zappy S1MINI2"
    if "CASTLE" in texto or "CASTTLE" in texto or "DYNAMO" in texto:
...
def calcular_16_por_ciento(equipo, precios=None):
    precio = obtener_precio_equipo(equipo, precios)
    return round(precio * 0.16, 2)
